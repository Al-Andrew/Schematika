...
Menu makeBlockMenu()
{
    ...
    for (auto t : allTypes)
    {
        ...
        b.displayBlocks.back().y = y - TITLE_HEIGHT;
        double dx = b.x - oldX;
        double dy = y - oldY-TITLE_HEIGHT;
...
void drawMenu(const Menu& m)
{
    ...
    slSetFontSize(TITLE_HEIGHT);
    slText(m.x, static_cast<double>(WINDOW_HEIGHT - TITLE_HEIGHT-TITLE_SPACER), "Blocks");
    slSetFontSize(20);
